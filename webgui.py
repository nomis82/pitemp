...
def create_table(rows):
    ...
    for row in rows[:-1]:
        rowstr="[{0}, {1}, {2}], \n".format(repr(datetime.strptime(str(row[0]), "%Y-%m-%d %H:%M:%S").strftime('new Date(%Y, %m-1, %d, %H, %M, %S)')),str(row[1]),str(row[2]))
        chart_table+=rowstr
...
def print_graph_script(rows):

    # google chart snippet ['Time', 'Temperature'],%s]);
    chart_code="""
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        dataView.addColumn('datetime', 'datetime');
      	dataView.addColumn('number', "Outdoor");
       	dataView.addColumn('number', "indoor");
        dataView.addRows([
%s]); 
    ...
    print chart_code % (rows)


        #var dateFormatter = new google.visualization.DateFormat({pattern: 'EEEE yyyy/MM/dd HH:mm'});
...
def main():
    ...
    printHTMLHead("Raspberry Pi Temperature Logger2", table)
